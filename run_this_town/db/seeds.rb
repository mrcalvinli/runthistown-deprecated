...
first_names = ["Calvin", "Daniel", "Christian", "Aaron", "Eric", "Sarah", "Emily", "Amy", "Victoria"]
last_names = ["Li", "Sanchez", "Mata", "Shirley", "Fong", "McGee", "Putin", "McSwaggerson"]
...
first_names.each do |first_name|
	last_names.each do |last_name|
		...
		age = 18 + rand(10)
		password = "swagswag" + age.to_s
		user = User.create(first_name: first_name, last_name: last_name, age: age, email: email, password: password, password_confirmation: password);

		id = user.id

		numRoutes = 1 + rand(4)
		(0..numRoutes).each do |i|
			listKeys = generateRandomKeys(2 + rand(3), 31)
			route = RunRoute.create(user_id: id, has_ran: false)
